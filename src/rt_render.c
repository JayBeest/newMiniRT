...
t_rt_vector	canvas_to_viewport(int x, int y, t_rt_scene *scene)
{
	...
	v.x = (float)x * scene->viewport.width / (float)scene->canvas.width;  //static divisions in a loop..
	v.y = (float)y * scene->viewport.height / (float)scene->canvas.height;
...
t_err	render_scene(t_rt_mlx *mlx, t_rt_scene *scene)
{
	...
	pixel.y = -scene->canvas.height / 2;
	while (pixel.y < scene->canvas.height / 2)
	{
		pixel.x = -scene->canvas.width / 2;
		while (pixel.x < scene->canvas.width / 2)
		{
...
//			color = trace_ray(scene->cameras[0].coordinates, D, scene); // needs matrix translation?
			mlx_put_pixel(mlx->img, pixel.x + scene->canvas.width / 2, scene->canvas.height - (pixel.y + scene->canvas.height / 2), color_to_int(color));
			pixel.x++;
		}
		pixel.y++;
	}
	...
	mlx->fps = mlx_put_string(mlx->mlx, fps, scene->canvas.width - 325, scene->canvas.height - 50);
